...
def build_dynamic_aggregation_pipeline(
    ...
    group_stage = {
        "$group": {
            "_id": None,
            "total_reviews": {"$sum": 1},  # Always include total count
        }
    }

    # Dynamically add filters for positive and negative counts
    if filters:
    ...
    # Add dynamic projection for positive and negative counts
    if filters:
...
def run_pipeline(user_query, location_ids, third_party_ids, company_ids, collection, top_k=50):
    ...
    if query_type == 'count':
        # Dynamically define filters based on user query
        filters = {}
